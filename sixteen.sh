...
APPLY_CAMERA2API_FIX "$FIRM_DIR/$TARGET_DEVICE"
...
BUILD_PROP "$FIRM_DIR/$TARGET_DEVICE" "system" "ro.build.display.id" "${D_ID} QuantumRom V-${VERSION}: Build by YazdanIrfan and AL Noman"
BUILD_PROP "$FIRM_DIR/$TARGET_DEVICE" "product" "ro.build.display.id" "${D_ID} QuantumRom V-${VERSION}: Build by YazdanIrfan and AL Noman"
